...
vg_sales['Sales'] = vg_sales['NA_Sales']+vg_sales['EU_Sales']+vg_sales['JP_Sales']+vg_sales['Other_Sales']
...
platform_sales = vg_sales.groupby(['Platform'])['Sales'].agg('sum').reset_index()
publisher_sales = vg_sales.groupby(['Publisher'])['Sales'].agg('sum').reset_index()
genre_sales = vg_sales.groupby(['Genre'])['Sales'].agg('sum').reset_index()
market_vg_sales = vg_sales.groupby(['Year'])[['NA_Sales', 'EU_Sales', 'JP_Sales', 'Other_Sales']].agg('sum').reset_index()
...
with c1:
    st.markdown('### Top 10')
    st.dataframe(vg_sales.head(10), hide_index = True, column_order=("Name", "Platform", "Sales")) 
with c2:
...
st.line_chart(market_vg_sales, x = 'Year', y = ['NA_Sales', 'EU_Sales', 'JP_Sales', 'Other_Sales'])
